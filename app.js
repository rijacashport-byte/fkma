...
function afficherDepenses(livre){
  ...
  lEl.innerHTML=all.map(d=>`<div class="depense-item">
    ...
    ${d.desc?`<div style="font-size:12px;color:var(--texte2);margin-top:2px">${d.desc}</div>`:''}
    ${d.auteur?`<div style="font-size:11px;color:var(--texte2)">Autorisé par : ${d.auteur}</div>`:''}
  </div>`).join('');
}
...
function exportExcelRakitra(){
  const d=snapRakitra();const wb=XLSX.utils.book_new();
...
async function ecrireGrandLivreFiang(d){
  ...
  // Raki-pandraisana (hors budget — info seulement)
  if(d.rpOn&&d.totalRP>0) ecritures.push({...base,es:'E',rubrique:'K45-INFO',libelle:'Raki-pandraisana (hors budget)',montant:d.totalRP,horsbudget:true});
...
function renderTags(){
  ...
  tagsListe.forEach(nom=>{
    ...
    tag.innerHTML=`${nom}<button onclick="supprimerTag('${nom.replace(/'/g,"\'")}')">×</button>`;
